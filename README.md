...
=> Pthread 조건변수 pthread_cond 를 사용하여 두 쓰레드의 활성 상태를 제어할 수 있다.
2. 단위 측정 시간과 현재 시간과의 차이점의 계산이 필요가 있다.
=> 시간 구조체 변수를 2개 선언하고 하나는 현재 시간를 계속 갱신한다.
=> 다른 하나는 비동기로 유지하면서 두 구조체의 값이 다를 때 memcpy()를 통해 이전 시간을 복사한다.
=> 그렇게 시간이 달라지면 파일에 기록한다.
